...
Loader.prototype._initTasks = function() {
	...
	//init all the tasks first - must be done to init dependencies before registering with other tasks below
	_.forEach(this._BigGulp.tasks, function(task, taskName) {
		task = _.merge(new BaseTask(), task);
		task.init(BigGulp, task, taskName);
	});

	//now add in the dependencies if needed
	_.forEach(this._BigGulp.tasks, function(task, taskName) {
		task = _.merge(new BaseTask(), task);
		task.registerDeps(BigGulp, task, taskName);
	});
};
